...
type StorySection={label:string;title:string;intro?:string;blocks:StoryBlock[];evidence?:EvidenceItem[];media?:StoryMedia[];ownership?:OwnershipItem[];artifact?:ArtifactItem;comparison?:ComparisonTable;demo?:boolean;interactive?:boolean;pmEvidence?:"threadline"|"zhixu"};
...
export default function ProjectCase({d}:{d:CaseData}){
  ...
  return <main className={`case-page ${d.demo}-case`}>
    ...
    <section className="case-hero">
      <div className="case-cover"><ImageLightbox className="case-cover-image" src={path(d.cover)} alt={`${d.name} 产品界面`}><img src={path(d.cover)} alt={`${d.name} 产品界面`}/></ImageLightbox><span className="case-cover-hint">横向滑动 / 点击放大 →</span></div>
      <div className="case-hero-main">
        <p className="case-kicker">{d.kicker}</p><h1>{d.name}</h1><h2>{d.tagline}</h2><p className="case-summary">{d.summary}</p>
        <CaseHeroMotion kind={d.demo}/>
    ...
    <div className="case-body"><article className="case-story">
      {d.story.map((section,index)=><section id={`section-${index+1}`} key={section.label}>
        ...
        {section.interactive&&<CaseInteractiveDemo kind={d.demo}/>}
        ...
        {section.media&&<div className={`case-media-grid case-media-${section.media.length}`}>{section.media.map(item=><figure key={item.src}><ImageLightbox className="case-media-frame" src={path(item.src)} alt={item.alt}><img src={path(item.src)} alt={item.alt}/>{item.markers?.map((marker,index)=><span className="case-marker" style={{left:`${marker.x}%`,top:`${marker.y}%`}} key={`${marker.label}-${index}`}><i>{index+1}</i>{marker.label}</span>)}</ImageLightbox><figcaption>{item.caption}<span>点击站内放大 ↗</span></figcaption></figure>)}</div>}
        {section.demo&&<div className="product-proof"><div className="product-proof-head"><div><span>当前产品界面</span><p>{d.demoIntro}</p></div>{d.links.map(link=>link.url&&<a key={link.label} href={link.url} target="_blank" rel="noreferrer">{link.label} ↗</a>)}</div><ImageLightbox className="product-proof-image" src={path(d.productImage)} alt={`${d.name} 当前真实产品界面`}><img src={path(d.productImage)} alt={`${d.name} 当前真实产品界面`}/></ImageLightbox><footer><span>{d.imageNote}</span><span>点击站内放大</span></footer></div>}
      </section>)}
    </article></div>
