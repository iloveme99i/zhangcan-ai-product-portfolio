...
export default function ZhixuJourneyDemo(){
  ...
  useEffect(()=>{
    ...
    const schedule:(readonly [number,()=>void])[]=[
      ...
      [4100,()=>{setConfirmed(true);setStep(2);setCondition(0)}],
  ...
  const reset=()=>{setStep(0);setActiveSource(null);setParsed(0);setCondition(0);setExpandedEvidence(null);setConfirmed(false);setTaskCreated(false);setHasEntered(true);setAutoPlaying(true)};
  ...
  return <section className="zhixu-journey" aria-label="职序五步交互演示" ref={rootRef}>
    ...
    <div className="zhixu-demo-workspace">
      ...
      <div className="zhixu-demo-panel" key={step}>
        ...
        {step===1&&<div><span className="demo-panel-kicker">02 / 岗位事实</span><h4>逐项核对 AI 提取结果</h4><p className="demo-panel-copy">移入或点击字段可定位原文。缺少的信息保持“需要确认”。</p><div className="fact-list">{facts.map((fact,index)=><button type="button" className={`${index<parsed?"visible":""} ${activeSource===fact.source?"active":""}`} onMouseEnter={()=>setActiveSource(fact.source)} onMouseLeave={()=>setActiveSource(null)} onFocus={()=>setActiveSource(fact.source)} onClick={()=>{takeControl();setActiveSource(fact.source)}} key={fact.name}><span>{fact.name}</span><b>{fact.value}</b><i className={fact.status==="已明确"?"ok":"pending"}>{fact.status}</i></button>)}</div><button type="button" className="demo-primary" onClick={()=>{takeControl();setConfirmed(true);setStep(2)}}>{confirmed?"岗位事实已确认":"确认岗位事实"} →</button></div>}
        ...
        {step===4&&<div><span className="demo-panel-kicker">05 / 投递行动</span><h4>{taskCreated?"投递任务已进入准备阶段":"建议进入投递准备"}</h4><p className="demo-panel-copy">结论不使用虚假的匹配分数，直接暴露待确认项和下一步。</p><ul className="action-list"><li>确认连续实习周期与到岗时间</li><li>针对岗位调整简历项目顺序</li><li>补充 SQL 项目证据</li><li>在截止时间前创建投递提醒</li></ul><button type="button" className={`demo-primary ${taskCreated?"complete":""}`} onClick={()=>{takeControl();setTaskCreated(true)}}>{taskCreated?"✓ 已创建 · 投递准备":"创建投递任务 →"}</button></div>}
      </div>
    </div>
